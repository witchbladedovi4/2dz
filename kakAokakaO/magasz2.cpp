...
//void ApplyLoyaltyDiscount(double& totalSum);
//void ApplySaleDiscount(double& totalSum);
//double cash = 1000;  // Пример наличных
//double emoney = 0;   // Пример электронных денег
//double cashMoney = 0; // Общая сумма наличных
//Функции
void Start();
bool Login();
void ShopUserMenu();
...
//--------------------------------
void ApplyLoyaltyDiscount(double& price) {
	price *= 0.9; // 10% скидка
	std::cout << "Применена скидка для постоянных клиентов: 10%\n";
}
void ApplySaleDiscount(double& price) {
	price *= 0.8; // 20% скидка
	std::cout << "Применена распродажная скидка: 20%\n";
}
void Selling()
{
	std::string chooseId, chooseCount, choosePay, chooseDisc;
	int id{}, count{};
	std::string userCash;
	bool isFirst = true;
	double totalSum = 0;
	...
	while (true)
	{
		...
		if (id == 0) 
		{
			if (!isFirst)
			{
				...
				std::cout << "Вы хотите применить скидку? (1 - Да, 2 - Нет): ";
				std::getline(std::cin, chooseDisc);

				if (chooseDisc == "1")
				{
					std::cout << "Выберите тип скидки:\n";
					std::cout << "1 - Для постоянных клиентов\n";
					std::cout << "2 - Распродажа\n";
					std::cout << "Ввод: ";
					std::getline(std::cin, chooseDisc);

					if (chooseDisc == "1")
						ApplyLoyaltyDiscount(totalSum);
					else if (chooseDisc == "2")
						ApplySaleDiscount(totalSum);
				}
					...
					else
					{
						if (std::stoi(choosePay) == 1)
						{
							while (true)
							{
								...
								{
									double cashInput = std::stod(userCash);
									if (cashInput < totalSum)
									{
